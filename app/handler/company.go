...
func (h *Handler) createCompany(_ http.ResponseWriter, r *http.Request) {
	...
	if errUser != nil {
		logError(fmt.Sprintf("error create company: %v", errUser))
	} else {
...
func (h *Handler) getCompany(_ http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logError(fmt.Sprintf("error get company by name %s: %v", companyName, err))
	} else {
...
func (h *Handler) delCompany(_ http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logError(fmt.Sprintf("error delete company by name %s: %v", companyName, err))
	} else {
...
func (h *Handler) updateCompany(_ http.ResponseWriter, r *http.Request) {
	...
	err := json.NewDecoder(r.Body).Decode(&c)
	if err != nil {
		logError(fmt.Sprintf("error decode patch body: %v", err))
	}

	if valErr := c.IsValid(); err != nil {
		logError(fmt.Sprintf("incorrect patch data: %v", valErr))
	}

	err = h.serviceCompany.Patch(&c)
	if err != nil {
		logError(fmt.Sprintf("error patch company %s: %v", c.Name, err))
	} else {
